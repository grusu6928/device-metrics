...
from src.repositories.metric_repository import AnomalyRepository, MetricRepository
...
class MetricsConsumer:
    ...
    async def _process_message(self, metric_data: Dict[str, Any]):
        ...
        try:
            logger.debug(
                f"Processing metric: {metric_data.get('device_id')} - {metric_data.get('metric_type')}"
            )
            ...
            metric = metric_repo.create_metric(
                {
                    ...
                    "timestamp": datetime.fromisoformat(
                        metric_data["timestamp"].replace("Z", "+00:00")
                    ),
