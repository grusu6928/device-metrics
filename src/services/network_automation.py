...
class NetworkDeviceManager:
    ...
    DEVICE_TYPES = ["router", "switch", "firewall", "load_balancer", "wireless_controller"]
    ...
    def validate_network_metric(
        self, device_id: str, metric_type: str, value: float, metadata: Dict
    ) -> bool:
    ...
    def _infer_device_type(self, metrics: List[Dict]) -> Optional[str]:
        ...
        if "bgp_peer_status" in metric_types or "routing_table_size" in metric_types:
            return "router"
        elif "interface_throughput" in metric_types and "switch" in str(
            metrics[0].get("metadata", {})
        ):
            return "switch"
...
class NetworkAutomationService:
    ...
    def generate_network_remediation(self, device_id: str, issue: Dict) -> Dict:
        ...
        if issue_type == "high_packet_loss":
            ...
            remediation["automation_script"] = self._generate_bgp_check_script(issue.get("peer"))
