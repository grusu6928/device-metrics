...
class HealthSummarizer:
    ...
    def summarize_device_health(self, device_id: str, db: Session,
                               recent_hours: int = 24) -> Dict:
        """
        ...
        cutoff_time = cutoff_time.replace(
            hour=cutoff_time.hour - recent_hours
        )
        
        metrics = db.query(DeviceMetric).filter(
            DeviceMetric.device_id == device_id,
            DeviceMetric.timestamp >= cutoff_time
        ).order_by(DeviceMetric.timestamp.desc()).limit(100).all()
        
        anomalies = db.query(Anomaly).join(DeviceMetric).filter(
            DeviceMetric.device_id == device_id,
            Anomaly.detected_at >= cutoff_time
        ).order_by(Anomaly.detected_at.desc()).limit(settings.MAX_ANOMALIES_FOR_SUMMARY).all()
    ...
    def _determine_health_status(self, metrics: List[DeviceMetric],
    ...
    def _generate_llm_summary(self, device_id: str, metrics: List[DeviceMetric],
                             anomalies: List[Anomaly], key_metrics: Dict) -> str:
...
Keep it professional and actionable."""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a DevOps engineer providing device health summaries."},
    ...
    def _generate_rule_based_summary(self, device_id: str, metrics: List[DeviceMetric],
                                    anomalies: List[Anomaly], key_metrics: Dict) -> str:
        ...
        if key_metrics:
            ...
            for metric_type, stats in list(key_metrics.items())[:3]:
                metric_parts.append(
                    f"{metric_type} at {stats['current']:.1f} "
