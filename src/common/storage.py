...
class ObjectStorageClient:
    ...
    def __init__(
        self, endpoint: str, access_key: str, secret_key: str, bucket: str, use_ssl: bool = True
    ):
    ...
    def archive_metrics(self, device_id: str, metrics_data: bytes, timestamp: str) -> Optional[str]:
        ...
        try:
            ...
            self.client.put_object(
                Bucket=self.bucket, Key=key, Body=metrics_data, ContentType="application/gzip"
            )
    ...
    def list_archives(self, device_id: Optional[str] = None, prefix: Optional[str] = None) -> list:
        ...
        try:
            ...
            response = self.client.list_objects_v2(Bucket=self.bucket, Prefix=search_prefix)
...
class StorageService:
    ...
    def archive_old_metrics(
        self, device_id: str, metrics: list, retention_days: int = 90
    ) -> Optional[str]:
        ...
        try:
            ...
            key = self.storage_client.archive_metrics(device_id, compressed, timestamp)
            return key
        except Exception as e:
